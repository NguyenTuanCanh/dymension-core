...
app.post("/faucet", jsonParser, async function (req, res) {
  try {
    const data = req.body;

    let address = await fs.promises.readFile("./store/address_faucet.json");
    address = JSON.parse(address);
    address.push(data.address);

    await fs.promises.writeFile("./store/address_faucet.json", JSON.stringify(address));

    res.send({ success: "Can wait up to 24 hours!" });
  } catch (error) {
    console.log(error);
    res.status(500).send({ error: "Something went wrong!" });
  }
